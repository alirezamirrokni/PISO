...
from .problem import PricingProblem, ProblemSpec
...
def _source_fingerprint(project_root: Path) -> str:
    # Only numerical implementation files belong in the expensive-run cache
    # fingerprint. Resolved algorithm parameters and problem settings are
    # already fingerprinted per task below, so changing suite labels, plotting,
    # validation, report settings, or which suites are active must not make
    # unrelated completed simulations run again.
    return _fingerprint_source_files(
        project_root,
        ["algorithm.py", "directions.py", "problem.py"],
    )


def _legacy_source_fingerprint(project_root: Path) -> str:
    # Compatibility with caches written by the previous codebase. Those caches
    # included config.py and suites.py in the source hash. Accept them once and
    # JobCache will transparently migrate them to the new stable fingerprint.
    return _fingerprint_source_files(
        project_root,
        ["algorithm.py", "directions.py", "problem.py", "config.py", "suites.py"],
    )
...
def _job_cache(task: Task) -> JobCache:
    ...
    # Previous releases did not include the input CSV in the task hash. Accept
    # those exact legacy fingerprints once so existing caches remain reusable;
    # JobCache then migrates them to the new fingerprint that does track data.
    compatible = [
        _task_fingerprint(
            task,
...
def _run_task(task: Task) -> dict[str, Any]:
    ...
    trace = run_piso_variant(
        ...
        week_id=task.week_id,
        run_index=task.run_index,
        # All variants in a suite use a common random stream. This makes the
        # comparisons paired without coupling unrelated suites.
...
def run_ablation(
    ...
    *,
    ...
    for suite_name in config["active_suites"]:
        ...
        if not plot_only:
            # Check the cache before creating workers. Fully cached suites never
            # enter the optimization runner at all; if one cache item is removed,
            # only that exact missing item is submitted for computation.
