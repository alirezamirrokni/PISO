...
from src.pricing_methods import METHODS
...
def run_experiment(config_path: Path, output_dir: Path, reset_cache: bool = False) -> None:
    ...
    if unknown:
        raise KeyError(f"Methods not registered in src/pricing_methods/__init__.py: {unknown}")

    plotting = config.get("plotting", {})
    plot_methods = list(plotting.get("methods", method_names))
    unknown_plot_methods = [name for name in plot_methods if name not in method_names]
    if unknown_plot_methods:
        raise KeyError(
            "Plotting methods must also appear under config['methods']: "
            f"{unknown_plot_methods}"
        )
    y_margin_fraction = float(plotting.get("y_margin_fraction", 0.05))
    if y_margin_fraction < 0.0:
        raise ValueError("plotting.y_margin_fraction must be nonnegative")

    y_limit_mode = str(plotting.get("y_limit_mode", "post_drop")).lower()
    if y_limit_mode not in {"post_drop", "final_window", "full"}:
        raise ValueError(
            "plotting.y_limit_mode must be 'post_drop', 'final_window', or 'full'"
        )

    post_drop_remaining_fraction = float(
        plotting.get("post_drop_remaining_fraction", 0.20)
    )
    if not 0.0 <= post_drop_remaining_fraction <= 1.0:
        raise ValueError(
            "plotting.post_drop_remaining_fraction must be in [0, 1]"
        )

    post_drop_tail_fraction = float(
        plotting.get("post_drop_tail_fraction", 0.25)
    )
    if not 0.0 < post_drop_tail_fraction <= 1.0:
        raise ValueError(
            "plotting.post_drop_tail_fraction must be in (0, 1]"
        )

    final_window_fraction = float(
        plotting.get("final_window_fraction", 0.30)
    )
    if not 0.0 < final_window_fraction <= 1.0:
        raise ValueError(
            "plotting.final_window_fraction must be in (0, 1]"
        )
    ...
    if cache.is_complete(required_outputs):
        print(
            f"Complete cache found in {output_dir}. "
            ...
            for run_index in range(simulations):
                ...
                for method_name in method_names:
                    ...
                    if getattr(method_class, "private_rng", False):
                        seed_alias = getattr(method_class, "seed_alias", method_name)
