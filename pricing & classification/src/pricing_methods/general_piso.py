...
def allocate_batches(params: dict[str, Any], iteration_batch: int) -> tuple[int, int, int]:
    ...
    if known <= 0 or plus <= 0 or minus <= 0:
        raise ValueError("PISO batch split must allocate at least one sample per component")
    return known, plus, minus


def known_gradient(problem, demands: np.ndarray) -> np.ndarray:
    return problem.partial_gradients(demands).mean(axis=0)
...
def haar_frame(rng: np.random.RandomState, dimension: int, columns: int) -> np.ndarray:
